...
class MergeWeightMask(object):
    ...
    def build_mask_subtree_only(cls, device, num_heads, key_pad, node_pad, spans, b, t, n, m, **kwargs):
        ...
        # node_pad_mask:        [b, 1, m, n + m]
        with torch.no_grad():
            key_pad = key_pad if key_pad is not None else torch.zeros(b, n, device=device, dtype=torch.bool)
            node_pad = node_pad if node_pad is not None else torch.zeros(b, m, device=device, dtype=torch.bool)
